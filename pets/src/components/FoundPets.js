          ...
          <div className="grid grid-cols-3 gap-4">
            {[...Array(12)].map((_, index) => (
              <div key={index} className="bg-gray-200 p-4 rounded-lg shadow-md">
                <img src={`https://picsum.photos/200/200?random=${index}`} alt={`Found pet ${index + 1}`} className="w-full h-32 object-cover rounded-md mb-4" />
                <h3 className="text-lg font-bold mb-2">Found {`{type}`}!</h3>
                <p>Type: Cat</p>
                <p>Gender: Female</p>
                <p>Breed: Unknown</p>
                <p>Color: Black</p>
